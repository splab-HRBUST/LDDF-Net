...
import random
from whisper_at.audio import log_mel_spectrogram, pad_or_trim, load_audio
...
class AudiosetDataset(Dataset):
    def __init__(self, dataset_json_file, audio_conf, label_csv=None,tdim=25):
    ...
    def _wav2fbank(self, filename, filename2=None, mix_lambda=-1):
        if 'whisper' in self.tar_path or 'ft' in self.tar_path:
            fmt = '.npz'
        else:
            fmt = '.npy'
        ...
        if filename2 == None:
            ...
            if feat.shape[1] < self.tdim:
                len_diff = self.tdim - feat.shape[1]
                feat = torch.nn.functional.pad(feat, (0, 0, 0, len_diff))
            else:
                feat = feat[:, :self.tdim, :]

        else:
            # filename = tar_path/4-102844-B-49.npz
            filename = tar_path + '.'.join(filename.split('/')[-1].split('.')[:-1]) + fmt
            feat = self.load_rep(filename)
            feat = torch.Tensor(feat)
            ...
            if feat.shape[1] < self.tdim:
                ...
                feat = feat[:, :self.tdim, :]
            if feat2.shape[1] < self.tdim:
                len_diff = self.tdim - feat2.shape[1]
                feat2 = torch.nn.functional.pad(feat2, (0, 0, 0, len_diff))
            else:
                feat2 = feat2[:, :self.tdim, :]
    ...
    def __getitem__(self, index):
        if random.random() < self.mixup:
            # self.data 是一个 [[./data/ESC-50-master/audio_16k/4-102844-B-49.wav,/m/07rwj49],........]
            datum = self.data[index]
            # 又变成一个字典 datum {wav:./data/ESC-50-master/audio_16k/4-102844-B-49.wav,labels:/m/07rwj49}
