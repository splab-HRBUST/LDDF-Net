...
parser.add_argument("--loss", type=str, default="BCE", help="the loss function for finetuning, depend on the task", choices=["BCE", "CE",'LT'])
...
#判断ghostnet的输入是32还是1280
# parser.add_argument('--ghost_in_channel', type=str, default='nlayer', choices=['nlayer', 'rep'],
#                     help='Whether to use 32 layers (nlayer) or rep_dim=1280 (rep) as GhostModule input channels')
...
parser.add_argument("--CosineAnnealingLR", type=ast.literal_eval, default='False', help="是否使用余弦退火")
parser.add_argument("--CosTmax", type=int, help="余弦的半个周期为多少个epoch")
parser.add_argument("--Cosmin", type=float, help="余弦退火的最小值学习率")
parser.add_argument("--tdim", type=str, default=50,help="特征的时间维度")
parser.add_argument("--train_tar_path", type=str,help="特征存放根路径")
parser.add_argument("--eval_tar_path", type=str,help="特征存放根路径")
parser.add_argument("--fold", type=str,help="fold几?")
args = parser.parse_args()
print(args.label_smooth)
if args.dataset == 'esc':
    if args.model_size == 'hubert-xlarge-ls960-ft' or args.model_size == 'wav2vec2-large-robust-ft-swbd-300h':
        train_tar_path = '/g813_u1/sml/EfficentWhisper/data' + args.model_size + 'tdim=' + args.tdim
        eval_tar_path = '/g813_u1/sml/EfficentWhisper/data' + args.model_size + 'tdim=' + args.tdim
    else:
...
if args.bal == 'bal' and args.dataset != 'esc': #是否采用平衡采样，因为Audioset中每个类不是一样多，所以必须根据类的权重采样才公平
    ...
    train_loader = torch.utils.data.DataLoader(
        dataloader.AudiosetDataset(args.data_train, label_csv=args.label_csv, audio_conf=audio_conf,tdim=args.tdim),
        batch_size=args.batch_size, shuffle=shuffle, num_workers=args.num_workers, pin_memory=True, drop_last=True)

val_loader = torch.utils.data.DataLoader(
    dataloader.AudiosetDataset(args.data_val, label_csv=args.label_csv, audio_conf=val_audio_conf,tdim=args.tdim),
...
if 'whisper-high' in args.model:
    ...
    # n_layer指原模型有多少层encoder，rep_dim指特征维度
...
code_path = args.exp_dir 
if os.path.exists(code_path) == False:
    os.mkdir(code_path)

copy_path = '/public/home/acal2okrm7997/g813_u1/sml/EfficentWhisper/src/whisper_at_train/esc-50'
try:
    # 使用subprocess.run代替os.system
    # check=True表示如果命令返回的状态码非0，则抛出CalledProcessError异常
    # shell=True允许执行shell命令
    subprocess.run('cp ' + copy_path + '/*.sh ' + code_path + '/..', check=True, shell=True)
